...
SUITE(parser)
{
    ...
    TEST(NeuronListTestCase)
    {
        ...
        CHECK_EQUAL("cZHH", neurons[0].first);
        CHECK_EQUAL(737, neurons[0].second);
        CHECK_EQUAL("eUNZ", neurons[1].first);
    ...
    TEST(LinkListTestCase)
    {
        ...
        CHECK_EQUAL("eYH", links[0].first.first);
        CHECK_EQUAL("z", links[0].first.second);
        CHECK_EQUAL(-68, links[0].second);
        CHECK_EQUAL("kT", links[1].first.first);
        CHECK_EQUAL("kUJ", links[1].first.second);
        CHECK_EQUAL(-1, links[1].second);
        CHECK_EQUAL("nZEGKXJ", links[2].first.first);
        CHECK_EQUAL("h", links[2].first.second);
        CHECK_EQUAL(37, links[2].second);
        CHECK_EQUAL("nUZYE", links[3].first.first);
